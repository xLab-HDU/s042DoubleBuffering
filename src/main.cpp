...
void Show_DoubleBuffer()
{
    ...
    if (BufferSwapFlag == false)
    {
        ...
        {
            //WriteConsoleOutputAttribute(hOutput1, &textColor, 1, coord, &bytes);
            coord.Y = i;
            WriteConsoleOutputCharacterA(hOutput1, ScreenData[i], mWidth, coord, &bytes);
        }
    ...
    else
    {
        ...
        for (i = 0; i < mHeight + 5; i++)
        {
            //WriteConsoleOutputAttribute(hOutput2, &textColor, 1, coord, &bytes);
            coord.Y = i;
            WriteConsoleOutputCharacterA(hOutput2, ScreenData[i], mWidth, coord, &bytes);
        }
